...
module.exports = class Client {
  ...
  connect () {
    return new Promise((resolve, reject) => {
      ...
      const connection = core.createConnection(settings, endpoint, name)
  ...
  async writeStream (stream, events, options) {
    ...
    events = events.map(e => core.createJsonEventData(
      e.eventId || uuid.v4(),
      e.data,
      e.metadata,
      e.eventType
  ...
  async connectPersistentSubscription (stream, group, eventHandler, dropHandler, options) {
    ...
    const onEventAppeared = async function (subscription, rawEvent) {
      ...
      try {
        ...
        event.nack(core.PersistentSubscriptionNakEventAction.Park, e.message)
      }
    }
  ...
  set options (options) {
    ...
    this._options = {
      ...
      credentials: new core.UserCredentials(
        options.credentials.username, options.credentials.password
      ),
      expectedVersion: options.expectedVersion || core.expectedVersion.any,
